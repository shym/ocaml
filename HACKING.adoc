...
To update the `master` branch on `ocaml/winpthreads` with new commits
from upstream `master` branch, you can do:

----
git clone https://github.com/ocaml/winpthreads.git
cd winpthreads
...
Finally you can update the `winpthreads` submodule, by running, in the
directory containing the `ocaml` repository:

----
git -C winpthreads fetch
git -C winpthreads checkout origin/master
git add winpthreads
----
