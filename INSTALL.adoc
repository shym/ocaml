...
A cross compiler is a compiler that runs on some machine, named the
_host_, but generate code for different machine, named the _target_. To
build a cross compiler you first need to have a non-cross compiler of
the same version installed in your `$PATH`. Note that they need to be of
the same version since the cross compilers will contain the runtime of
the non-cross compiler because the C toolchain that will be detected
during the `configure` for the OCaml cross compiler will be the C cross
compiler to `target`. It may be useful to keep a note of the flags used
to link the non-cross compiler with `zstd`, if any:
