...
/* Operations
   ...
   - encode-C-utf8-literal. Produces a valid string for Unix, regardless of the
     running OS.

   - encode-C-utf16-literal. Produces a valid string for Windows, regardless of
     the running OS.
 */
...
#define escapeable(c) ((c) == '\\' || (c) == '"' || (c) == '\n' || (c) == '\r')

#define print_escapable(c)    \
    if ((c) == '\\') {        \
      printf("\\\\");         \
    } else if ((c) == '"') {  \
      printf("\\\"");         \
...
int main_os(int argc, char_os **argv)
{
  if (argc == 3 && !strcmp_os(argv[1], T("encode-C-literal"))) {
    encode_C_literal(argv[2], default_utf8);
  } else if (argc == 3 && !strcmp_os(argv[1], T("encode-C-utf8-literal"))) {
